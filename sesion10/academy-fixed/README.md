...
# Arrancar solo Mongo Express (el resto del stack ya debe estar corriendo)
...
# Abrir shell de MongoDB dentro del contenedor
docker compose exec mongodb-read mongosh academy_read
...
# Buscar por campo
db.students.find({ email: "ana@academy.com" }).pretty()


# Conectar a PostgreSQL y listar las tablas
docker compose exec postgres-write psql -U academy -d academy_write -c "\dt"

# Consulta directa sin entrar al shell interactivo
docker compose exec postgres-write psql -U academy -d academy_write \
  -c "SELECT * FROM students;"

docker compose exec postgres-write psql -U academy -d academy_write \
  -c "SELECT * FROM enrollments;"
...
\d students                  # ver estructura de una tabla concreta
\d+ enrollments              # ver estructura detallada con tamaños
...
# Ver si hay algún domain event siendo publicado
docker compose logs academy-api | grep -i "domain\|event\|enrollment\|publish"

# Listar todas las colas con su número de mensajes
curl -s -u guest:guest http://localhost:15672/api/queues | jq '.[] | {name, messages, messages_ready, messages_unacknowledged}'

# Ver una cola concreta
curl -s -u guest:guest http://localhost:15672/api/queues/%2F/AlumnoMatriculadoEvent | jq .

# Ver todos los exchanges
curl -s -u guest:guest http://localhost:15672/api/exchanges | jq '.[] | {name, type}'

# Ver los mensajes publicados/consumidos por segundo (estadísticas)
curl -s -u guest:guest http://localhost:15672/api/queues | jq '.[] | {name, message_stats}'

# Ver mensajes en cola cada 2 segundos
watch -n 2 'curl -s -u guest:guest http://localhost:15672/api/queues \
  | jq ".[] | {name, messages, consumers}"'

# Terminal 1 — monitorizar las colas en tiempo real
watch -n 1 'curl -s -u guest:guest http://localhost:15672/api/queues | \
  jq ".[] | {name: .name, messages: .messages, delivered: .message_stats.deliver_get}"'

# Terminal 2 — crear un estudiante y matricularlo
curl -s -X POST http://localhost:5000/api/students \
  -H "Content-Type: application/json" \
  -d '{"firstName":"Test","lastName":"Outbox","email":"test.outbox@test.com","dateOfBirth":"2000-01-01T00:00:00Z"}' | jq .

# Ver si el estudiante llegó a MongoDB
docker compose exec mongodb-read mongosh academy_read --quiet \
  --eval 'db.students.find({email: "test.outbox@test.com"}).pretty()'
  
# Ver el cuerpo de los mensajes de una cola en tiempo real desde los logs
docker compose logs -f academy-api | grep -i "publish\|consumer\|AlumnoMatriculado\|rabbitmq"
